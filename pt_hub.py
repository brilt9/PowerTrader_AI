...
class PowerTraderHub(tk.Tk):
    ...
    def open_settings_dialog(self) -> None:
        ...
        # --- Crypto.com API setup (writes crypto_key.txt + crypto_secret.txt used by pt_trader.py) ---
        def _api_paths() -> Tuple[str, str]:
            key_path = os.path.join(self.project_dir, "crypto_key.txt")
            secret_path = os.path.join(self.project_dir, "crypto_secret.txt")
        ...
        def _refresh_api_status() -> None:
            ...
            missing = []
            if not k:
                missing.append("crypto_key.txt (API Key)")
            if not s:
                missing.append("crypto_secret.txt (Secret Key)")
        ...
        def _open_api_folder() -> None:
            """Open the folder where crypto_key.txt / crypto_secret.txt live."""
            try:
        ...
        def _clear_api_files() -> None:
            """Delete crypto_key.txt / crypto_secret.txt (with confirmation)."""
            key_path, secret_path = _api_paths()
            if not messagebox.askyesno(
                ...
                f"  {key_path}\n"
                f"  {secret_path}\n\n"
                "After deleting, the trader can NOT authenticate until you configure new API credentials.\n\n"
            ...
            _refresh_api_status()
            messagebox.showinfo("Deleted", "Deleted crypto_key.txt and crypto_secret.txt.")

        def _open_cryptocom_api_wizard() -> None:
            """
            Beginner-friendly wizard for Crypto.com Exchange API credentials.

            What we store:
              - crypto_key.txt    = your Crypto.com API Key
              - crypto_secret.txt = your Secret Key (treat like a password)
            """
            import webbrowser
            ...
            wiz = tk.Toplevel(win)
            wiz.title("Crypto.com API Setup")
            ...
            key_path, secret_path = _api_paths()
            existing_api_key, existing_secret = _read_api_files()
            ...
            # Instructions
            intro = (
                "Configure your Crypto.com Exchange API credentials.\n\n"
                "📋 How to get API keys:\n"
                "  1) Go to https://crypto.com/exchange\n"
                "  2) Log in and complete KYC verification\n"
                "  3) Navigate to Settings → API Keys\n"
                "  4) Click '+ Create API Key'\n"
                "  5) Set permissions: Read ✓ and Trade ✓\n"
                "  6) Save the API Key and Secret Key\n\n"
                "This wizard will save two files:\n"
                "  - crypto_key.txt (your API Key)\n"
                "  - crypto_secret.txt (your Secret Key) ← keep this private!"
            )

            intro_lbl = ttk.Label(container, text=intro, justify="left")
            intro_lbl.grid(row=0, column=0, sticky="ew", pady=(0, 15))

            # Button to open Crypto.com
            top_btns = ttk.Frame(container)
            top_btns.grid(row=1, column=0, sticky="ew", pady=(0, 15))

            def open_cryptocom():
                webbrowser.open("https://crypto.com/exchange/user/settings/api-management")

            def open_docs():
                webbrowser.open("https://exchange-docs.crypto.com/exchange/v1/rest-ws/index.html")

            ttk.Button(top_btns, text="Open Crypto.com API Settings", command=open_cryptocom).pack(side="left")
            ttk.Button(top_btns, text="API Documentation", command=open_docs).pack(side="left", padx=8)

            # API Key entry
            key_frame = ttk.LabelFrame(container, text="API Key")
            key_frame.grid(row=2, column=0, sticky="ew", pady=(0, 10))
            key_frame.columnconfigure(0, weight=1)

            api_key_var = tk.StringVar(value=existing_api_key or "")
            ttk.Entry(key_frame, textvariable=api_key_var).grid(row=0, column=0, sticky="ew", padx=10, pady=10)

            # Secret Key entry
            secret_frame = ttk.LabelFrame(container, text="Secret Key")
            secret_frame.grid(row=3, column=0, sticky="ew", pady=(0, 10))
            secret_frame.columnconfigure(0, weight=1)

            secret_key_var = tk.StringVar(value=existing_secret or "")
            secret_entry = ttk.Entry(secret_frame, textvariable=secret_key_var, show="*")
            secret_entry.grid(row=0, column=0, sticky="ew", padx=10, pady=10)

            # Test button
            def _test_credentials() -> None:
                api_key = (api_key_var.get() or "").strip()
                secret = (secret_key_var.get() or "").strip()

                if not requests:
                    messagebox.showerror(
                        "Missing dependency",
                        "The 'requests' package is required for testing.\n\n"
                        "Fix: pip install requests"
                    )
                    return

                if not api_key or not secret:
                    messagebox.showerror("Missing credentials", "Please enter both API Key and Secret Key.")
                    return
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        if data.get("code") == 0:
                            price = "N/A"
                            try:
                                price = data.get("result", {}).get("data", [{}])[0].get("a", "N/A")
                            except Exception:
                                pass
                            messagebox.showinfo(
                                "Connection successful",
                                f"✅ Crypto.com API is reachable!\n\n"
                                f"BTC_USDT ask price: ${price}\n\n"
                                "Your credentials format looks valid.\n"
                                "Click Save to store them."
                            )
                        else:
                            messagebox.showwarning("API Warning", f"API returned: {data}")
                    else:
                        messagebox.showerror("Test failed", f"HTTP {resp.status_code}: {resp.text}")
                except Exception as e:
                    messagebox.showerror("Test failed", f"Could not connect to Crypto.com:\n\n{e}")

            test_btns = ttk.Frame(container)
            test_btns.grid(row=4, column=0, sticky="w", pady=(0, 15))
            ttk.Button(test_btns, text="Test Connection", command=_test_credentials).pack(side="left")

            # Acknowledgment checkbox
            ack_var = tk.BooleanVar(value=False)
            ack = ttk.Checkbutton(
                container,
                text="I understand crypto_secret.txt is PRIVATE and I will not share it.",
                variable=ack_var,
            )
            ack.grid(row=5, column=0, sticky="w", pady=(0, 10))

            # Save/Cancel buttons
            save_btns = ttk.Frame(container)
            save_btns.grid(row=6, column=0, sticky="ew")
            ...
            def do_save():
                ...
                if not api_key:
                    ...
                    return
                if not secret:
                    messagebox.showerror("Missing Secret Key", "Please enter your Secret Key.")
                    return
                if not bool(ack_var.get()):
                    messagebox.showwarning(
                        "Please confirm",
                        "Please check the box confirming you understand the secret key is private."
                    )
                    return
                ...
                try:
                    ...
                    messagebox.showerror("Save failed", f"Couldn't write credential files:\n\n{e}")
                    return
                ...
                messagebox.showinfo(
                    ...
                    "The trader will use these credentials automatically."
                )
                wiz.destroy()

            ttk.Button(save_btns, text="Save", command=do_save).pack(side="left")
            ttk.Button(save_btns, text="Cancel", command=wiz.destroy).pack(side="left", padx=8)

        ttk.Label(frm, text="Crypto.com API:").grid(row=r, column=0, sticky="w", padx=(0, 10), pady=6)
        ...
        ttk.Button(api_row, text="Setup Wizard", command=_open_cryptocom_api_wizard).grid(row=0, column=1, sticky="e", padx=(10, 0))
